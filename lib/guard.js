...
export function createGuard(stateStore, config, log) {
  ...
  return {
    ...
    canCheck(now = Date.now()) {
      // An open breaker still permits a sparse probe so network recovery
      // does not require deleting state.json or restarting the host.
      return s().level !== "off" || now - (s().lastCheckAt ?? 0) >= Math.max(300000, config.checkIntervalMs);
    },
