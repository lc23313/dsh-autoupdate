...
export const CONFIG_DEFAULTS = {
  ...
  autoApply: true,
  /** NEW v1.1.0 — false = manual mode: no silent periodic checks; version
   * detection only runs when the user clicks 检查更新 in Settings (the
   * original check/arm/rollback logic itself is unchanged). */
  autoCheck: false,
...
export class AutoUpdater {
  ...
  start() {
    ...
    // NEW v1.1.0 — manual mode gate: timers only when autoCheck is true.
    if (this.config.autoCheck === true) {
      ...
      this.info("manual mode (autoCheck=false): no silent checks; detection runs only via the Settings UI button");
    }
  ...
  async checkNow(reason) {
    if (!this.config.enabled || this.stopped || this._checking) return;
    ...
    try {
      ...
      if (e?.updatePhase !== "apply") this.guard.recordFailure(e, "check");
    } finally {
      this._checking = false;
    }
  }
  ...
  async _check(reason) {
    ...
    if (!this.state.data.lastFailureAt) this.guard.recordSuccess();
  ...
  async armHelper(fromVersion, targetVersion) {
    if (!this.config.enabled || this.stopped) throw new Error("插件已禁用或停止");
    if (!this.state.data.npmPrefix) throw new Error("无法确定当前 dsh 的安装前缀，请使用全局 npm 安装的 dsh 启动后重试");
    if (!existsSync(HELPER_PATH)) throw new Error("更新助手脚本缺失，请重新安装插件");
    const existing = this.state.data.helper;
    if (existing?.armed) {
      if (existing.targetVersion === targetVersion && processAlive(existing.helperPid)) {
    ...
    const profiles = this.config.updateProfilePlugins ? await this.detectProfiles() : [];
    ...
    try {
      const persisted = JSON.parse(readFileSync(join(this.dataDir, "state.json"), "utf8"));
      if (persisted.helper?.armId !== armId) throw new Error("无法保存更新计划，请检查插件数据目录的写入权限");
      ...
      this.state.mutate((d) => { d.helper.helperPid = child.pid; });
      child.unref();
      this.info(
  ...
  consumeHelperResult() {
    ...
    if (res && ["waiting", "installing", "verifying", "profiles"].includes(res.phase)) return;
    safe(() => unlinkSync(file));
    if (!res || typeof res !== "object") return;
    if (res.armId && this.state.data.helper?.armId && res.armId !== this.state.data.helper.armId) return;
  ...
  staleHelperSweep() {
    ...
    if (!h?.armed) return;
    if (!processAlive(h.helperPid) && !processAlive(h.parentPid)) {
      this.warn("clearing stale armed helper (helper and parent no longer running)");
      this.state.mutate((d) => {
        d.helper = null;
      });
    }
  ...
  // ---- NEW v1.1.0: UI-facing manual operations ----

  /** Snapshot for the Settings page (status endpoint). */
  getUiStatus() {
    const d = this.state.data;
    return {
      enabled: this.config.enabled && !this.stopped,
  ...
  /**
   * Manual check for the UI button. Runs the same detection pipeline as
   * _check but returns a structured result and never arms the helper —
   * applying an update from the UI requires the explicit confirm step
   * (armFromUi). Circuit-breaker bookkeeping matches the periodic path.
   */
  async checkForUi() {
    if (!this.config.enabled || this.stopped) throw new Error("插件已禁用或停止");
    if (this._checking) throw new Error("已有检测正在进行，请稍候");
    this._checking = true;
    try {
      const cur = await currentDshVersion({
        npmCommand: this.config.npmCommand,
      ...
      if (!cur.version) throw new Error("无法确定当前 dsh 版本（manifest / dsh --version 均失败）");
      this.state.mutate((d) => {
        d.npmRoot = cur.npmRoot ?? d.npmRoot;
        d.npmPrefix = cur.prefix ?? null;
        d.installedVersion = cur.version;
      ...
      const target = tags[this.config.channel] ?? tags.latest ?? null;
      if (!target || !semver.valid(target)) {
        throw new Error(`registry 在通道 "${this.config.channel}" 上返回了不可用的版本信息: ${JSON.stringify(target)}`);
      }
      this.state.mutate((d) => {
        d.lastCheckAt = Date.now();
        d.lastCheckOk = true;
      ...
      if (!semver.gt(target, cur.version)) {
        ...
        this.guard.recordSuccess();
        this.info(`ui check: up to date (${cur.version}, channel "${this.config.channel}")`);
        return { state: "up-to-date", current: cur.version, latest: target, channel: this.config.channel, checkedAt: Date.now() };
      }
      this.state.mutate((d) => {
        d.pendingVersion = target;
      });
      if (!this.state.data.lastFailureAt) this.guard.recordSuccess();
      this.info(`ui check: update available ${cur.version} → ${target} (channel "${this.config.channel}")`);
      return {
      ...
      this.error(`ui check failed: ${e?.message ?? e}`);
      throw e;
    } finally {
      this._checking = false;
    }
  ...
  /**
   * Arm the exit-time helper from the UI's 确认更新 button. The target must
   * equal the version the last check resolved (state.pendingVersion) — no
   * free-form installs through the UI.
   */
  async armFromUi(targetVersion) {
    if (!this.config.enabled || this.stopped) throw new Error("插件已禁用或停止");
    if (this._checking || this._arming) throw new Error("已有更新操作正在进行，请稍候");
    if (typeof targetVersion !== "string" || !semver.valid(targetVersion)) {
      throw new Error(`无效的目标版本: ${JSON.stringify(targetVersion)}`);
    }
    const pending = this.state.data.pendingVersion;
    if (pending !== targetVersion) {
      throw new Error(`目标版本与检测结果不一致（待更新版本: ${pending ?? "无"}），请重新检查更新`);
    }
    const cur = this.state.data.installedVersion;
    if (!cur) throw new Error("当前版本未知，请重新检查更新");
    if (!semver.gt(targetVersion, cur)) throw new Error("目标版本必须高于当前版本");
    this._arming = true;
    try {
      await this.armHelper(cur, targetVersion);
    } finally {
      this._arming = false;
    }
    const h = this.state.data.helper;
    if (!h?.armed || h.targetVersion !== targetVersion) {
      throw new Error("更新助手武装失败，请查看日志（plugins-data/dsh-autoupdate/autoupdate.log）");
    }
    return { armed: true, targetVersion, fromVersion: cur };
  }
}
