 ...
 */
export function resolveRunningDsh() {
  try {
    const entry = process.argv[1];
    if (!entry) return null;
    const p = resolve(entry);
    // <prefix>[/lib]/node_modules/@deepseek-ai/dsh/<bin-or-lib>/…/<entry>
    const pkgDir = dirname(dirname(dirname(p)));
    const manifest = JSON.parse(readFileSync(join(pkgDir, "package.json"), "utf8"));
    if (manifest.name !== "@deepseek-ai/dsh") return null;
    const root = dirname(dirname(pkgDir)); // …/node_modules
    const rootParent = dirname(root); // <prefix> (win) or <prefix>/lib (unix)
    const prefix = basename(rootParent) === "lib" ? dirname(rootParent) : rootParent;
    return {
      packageDir: pkgDir,
...
export async function currentDshVersion({
  ...
  if (m) return { version: m[0], via: "dsh --version", prefix: null, npmRoot: root };
  if (root) {
    const v = readPackageVersion(join(root, "@deepseek-ai", "dsh"));
    if (v) return { version: v, via: "npm-root-manifest", prefix: dirname(root), npmRoot: root };
  }
...
export async function verifyDshInstall(prefix, expectedVersion, { timeoutMs = 30000 } = {}) {
  try {
    ...
    if (!binRel) return { ok: true, reason: "manifest verified (no bin declared)" };
